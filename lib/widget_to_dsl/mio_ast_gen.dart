...
import 'mio_ast_check_gen.dart';
import 'mio_check_node_map.dart';

@AstNodeCheck('lib/mio/widget_to_dsl/mio_ast_gen.dart')
...
class CustomAstVisitor extends SimpleAstVisitor<Map> {
  Map _visitNode(AstNode node) {
    if (!checkNode.containsKey(node.runtimeType.toString())) {
      stdout.writeln(
          '不支持的节点${node.runtimeType}<---->${node.parent!.toSource()}---->${node.toSource()} ');
    }
  ...
  Map? visitVariableDeclaration(VariableDeclaration node) {
    return _buildVariableDeclaration(_visitNode(node.name), _visitNode(node.initializer!));
  }
  ...
  Map? visitVariableDeclarationList(VariableDeclarationList node) {
    return _buildVariableDeclarationList(_visitNode(node.type!), _visitNodeList(node.variables),
...
//  }
  ...
  Map? visitFunctionDeclaration(FunctionDeclaration node) {
    return _buildFunctionDeclaration(_visitNode(node.name), _visitNode(node.functionExpression));
  }
  ...
  Map? visitFunctionExpression(FunctionExpression node) {
    return _buildFunctionExpression(_visitNode(node.parameters!), _visitNode(node.body),
        isAsync: node.body.isAsynchronous);
  }

  @override
  Map? visitSimpleFormalParameter(SimpleFormalParameter node) {
    return _buildSimpleFormalParameter(_visitNode(node.type!), node.identifier?.name ?? '');
  }
...
//  };
  ...
  //方法声明
  Map _buildMethodDeclaration(Map id, Map parameters, Map typeParameters, Map body, Map returnType,
