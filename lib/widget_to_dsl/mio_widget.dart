...
import 'package:mio/widget_to_dsl/mio_transformer.dart';
...
Future<String> parseDir(Directory dir) async {
  var files = await dir
      ...
      .where((event) =>
          event.endsWith('.dart') &&
          (transformer.skipSource.indexWhere((skip) => event.endsWith(skip)) == -1))
      .toList();
...
Future<void> _analyzeFramework(List<String> paths) async {
  ...
  for (var path in paths) {
    ...
    var w = await processFile(context, path);
    if (w.components.isEmpty) continue;
    widgets.addAll(w.components);
    var n = _generateFileName(path);
    // name shrinking
    var index = n.indexOf('_');
    if (index != -1) {
      var s = shrinking.entries
          .firstWhere((element) => n.startsWith(element.key), orElse: () => MapEntry('a', 'a'));
      n = s.value;
    }
    w.aliasGroup = n;
    components.add(w);
  ...
  shrinking.forEach((key, value) {
    ...
    var buffer = StringBuffer('''// This file is generated by Mio, do not edit manually!\n
import 'package:flutter/${isCupertino ? "cupertino.dart" : "material.dart"}';\n''');

    var allImports = c.fold<Set<String>>(<String>{}, (p, e) {
      p.addAll(e.imports);
      return p;
    });
    allImports.forEach((element) => buffer.writeln('import \'$element\';'));

    var l = c.fold<Set<String>>(<String>{}, (p, e) {
      p.addAll(e.lines);
      return p;
    });
    var lines = l.toList();
    ...
    var body = c.fold<Set<String>>(<String>{}, (p, e) {
      p.add(e.body);
      return p;
    });
...
Future<String> _generateWidget({List<ComponentParts>? components}) async {
  var c = components;
  if (c == null) return '';
  var isCupertino = c.first.isCupertino;
  var buffer = StringBuffer('''
...
Future<void> _generateSdkFile(String sdkName, String filePath, String output) async {
  ...
  await saveIntoFile(buffer.toString(), output, 'mio_${clzName}.dart');
}
...
Map<String, dynamic> _writeMethod(
    StringBuffer buffer, String name, Method element, Map<String, dynamic> defaultCache) {
  ...
  if (element.parameters != null && element.parameters.isNotEmpty) {
    ...
    var params = element.parameters
        .fold('', (value, p) => value + (p.isNamed ? '${p.type} ${p.name}, ' : '${p.name}, '));
    print('➡️ $name({$params})');
  } else {
    print('➡️ $name()');
  }
...
bool _tryInternalCheck(InterfaceType type, List<String> widgets, classElement) {
  try {
    if (type.superclass != null &&
      ...
      var superName =
          (classElement.linkedNode as ClassDeclarationImpl).extendsClause.superclass.name.name;
...
class ClassExposed extends Exposed {
  ...
  int get size => constructor.length + fields.length + functionParameters.length;
}
...
bool _invalidElement(Element element) {
  ...
  var isVisibleForTesting = meta.isNotEmpty
      ? meta.where(
          (element) => element.isVisibleForTesting || element.toSource() == '@visibleForTesting')
      : null;
...
List<ClassExposed> _visit(CompilationUnitElement unitElement, [bool isSdk = false]) {
  ...
  for (var classElement in apis) {
    ...
    var isWidget = classElement.thisType is InterfaceType
        ? _matchType(classElement.thisType, baseWidget, classElement: classElement)
        : false;
    ...
    if (isWidget || isAPI || isSdk) {
      ...
      for (var constructorElement in classElement.constructors) {
        if (!_invalidElement(constructorElement)) {
          String name;
          if (constructorElement.name == null || constructorElement.name == '') {
            ...
            constructorElement.parameters.forEach((e) {
              ...
              if (e.type.name == null && e.type is FunctionType) {
                ...
                functionParameters.add(FunctionParameter(e.name,
                    ...
                    returnType: (e.type as FunctionType).returnType.name ?? 'void',
                    typeArgument: ((e.type as FunctionType)?.typeArguments?.isNotEmpty ?? false)
